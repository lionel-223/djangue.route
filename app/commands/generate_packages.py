...
def compute_limit_date_letters():
    ...
    if MAX_MONTHS_SINCE_LETTER < today.month:
        return today.replace(
            month=(today.month - MAX_MONTHS_SINCE_LETTER),
...
def generate_packages():
    ...
    recipients = db.session.query(Recipient).filter(
        (Recipient.nb_letters > 0) & (Recipient.status == Recipient.Status.approved)
    )
    limit_date_letters = compute_limit_date_letters()
    for recipient in recipients:
        if not recipient.needs_new_package:
            continue
        ...
        html_package = [
            {
                "html": render_template(
                    'letter.html',
                    letter=letter, logo_path=LOGO_PATH, css_path=LETTER_CSS,
                    image_path=os.path.join(
                        app.FILE_UPLOAD_FOLDER,
                        letter.upload.hash + letter.upload.extension
                    ) if letter.upload else None
                ),
                "id": letter.id,
        ...
        send_email(
            subject="Vos lettres 1 Lettre 1 Sourire",
